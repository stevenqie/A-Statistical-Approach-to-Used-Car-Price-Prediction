...
results <- data.frame(cp = numeric(), minsplit = integer(), maxdepth = integer(), MSE = numeric())
...
for (cp in cp_values) {
  for (minsplit in minsplit_values) {
    for (maxdepth in maxdepth_values) {
      ...
      # Calculate MSE
      mse <- sqrt(mean((test_data$price - predictions)^2))
      
      # Store results
      results <- rbind(results, data.frame(cp = cp, minsplit = minsplit, maxdepth = maxdepth, MSE = mse))
    }
  }
}
...
rpart_model <- rpart(
        price ~ .,
        data = train_data,
        control = rpart.control(cp = best_params$cp, minsplit = best_params$minsplit, maxdepth = best_params$maxdepth)
      )
importance <- rpart_model$variable.importance

importance_sorted <- sort(importance, decreasing = TRUE)

barplot(importance_sorted, main = "Variable Importance", col = "blue", las = 2)
print(best_params)
```

Our final regression tree model has cp = 0.001, minsplit = 10, and maxdepth = 10. This model achieves an RMSE of 11368.41.

One of the advantages of regression trees is its interpretability. The plot of variable importance shows that the top influenteial variables in descending order are mileage, horsepower, model year, and cylinders. These results also somewhat overlap in the importance of variables between our linear model and random forest.

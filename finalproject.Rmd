...
```{r, echo=FALSE}
summary(data_one_hot[, c("model_year", "mileage", "price", "horsepower", "displacement")])
```
...
best_params <- results[which.min(results$MSE), ] #outputs cp = 0.001, minsplit = 10, maxdepth = 15 

#train with final model 
rpart_model <- rpart(
        price ~ .,
        data = train_data,
...
dummy_model <- dummyVars(~ ., data = data)
data_one_hot2 <- as.data.frame(predict(dummy_model, newdata = data))

#sapply(data_one_hot, class)
```
```{r, echo=FALSE}
X = as.matrix(data_one_hot2[, -1])
y = as.vector(data_one_hot2$price)

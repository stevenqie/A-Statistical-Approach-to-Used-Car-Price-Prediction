...
First, let's import the data into our workspace
```{r}
...
usedcars.kmean <- kmeans(data[, -ncol(data)])
```
...
```{r}
hclust_simple <- hclust(dist(train[, -ncol(data)], method="euclidean"), method = "single")
hclust_complete <- hclust(dist(train[, -ncol(data)], method = "euclidean"), method = "complete")
hclust_average <- hclust(dist(train[, -ncol(data)], method = "euclidean"), method = "average")
```
...
First, we are going to split our data into training and testing datasets. 
```{r}
...
1. Linear Model. We will be using elastic net regression, which is 
```{r}
lm_model <- lm(train_labels ~ ., data = as.data.frame(train_data))

# Make predictions on the test set
lm_predictions <- predict(lm_model, newdata = as.data.frame(test_data))
```
2. KNN 
3. Random Forest 
```{r}

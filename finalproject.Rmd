...
# One-hot encoding
transmission_dummies <- model.matrix(~ transmission - 1, data = data)
transmission_dummies <- as.data.frame(transmission_dummies)

colnames(transmission_dummies) <- gsub("transmission", "transmission type: ", colnames(transmission_dummies))

# summary(transmission_dummies)

# Add the dummy columns to the original dataframe
data <- cbind(data, transmission_dummies)

# factor transmission as well

data$transmission_factored <- factor(data$transmission)
```

## cylinders
```{r}
# One-hot encoding
cyl_dummies <- model.matrix(~ cylinders_factor - 1, data = data)
cyl_dummies <- as.data.frame(cyl_dummies)

colnames(cyl_dummies) <- gsub("cylinders_factor", "cylinder number ", colnames(cyl_dummies))

# summary(cyl_dummies)

data <- cbind(data, cyl_dummies)
```
...
```{r}
# One-hot encoding
fuel_dummies <- model.matrix(~ fuel_type_factor - 1, data = data)
fuel_dummies <- as.data.frame(fuel_dummies)

colnames(fuel_dummies) <- gsub("fuel_type_factor", "fuel type ", colnames(fuel_dummies))

data <- cbind(data, fuel_dummies)
```
...
numeric_data_without_price_scaled <- scale(numeric_data_without_price)
```
...
hclust_single <- hclust(dist(numeric_data_without_price_scaled, method = "euclidean"), method = "single")
hclust_complete <- hclust(dist(numeric_data_without_price_scaled, method = "euclidean"), method = "complete")
hclust_average <- hclust(dist(numeric_data_without_price_scaled, method = "euclidean"), method = "average")
...
```{r}
# Cut the dendrogram into 4 clusters using single linkage
clusters_single <- cutree(hclust_single, k = 2)

# Assign cluster labels to the dataset
data$cluster <- as.factor(clusters_single)

...
```
These are all numeric features as clustering requires numeric features
...
```
We will use complete linkage as it looks like it generates pretty distinct clusters
...
Finally, we will try spectral clustering, which aims to group observations based on their proximity information. This method involves 2 main steps, the first being using the eigenvalues of a similarity matrix to perform dimension reduction, followed by applying a clustering algorithm like K-means. The same features as hierarchical clustering will be used
...
ggplot(data %>% filter(!is.na(cluster)), aes(x = model_year, y = price, color = cluster)) +
  geom_point(size = 2) +
  labs(title = "Spectral Clustering Results", x = "Model Year", y = "Price") +
  theme_minimal()
...
### 4. Random Forest 
Random Forest is another powerful supervised machine learning algorithm that involves building numerous decision trees using different subsets of the data, acquired through a technique called bootstrapping.  
```{r echo = FALSE}
set.seed(1)
...
```
We use MSE to evaluate random forest since that metric is directly comparable with other models. 
Base random forest gets MSE (not OOB error) is 9855.106 with ntree = 500 and all other parameters being default. 
Tuned random forest gets MSE of 9497.301 with ntree = 500, mtry = 5, nodesize = 1, and sampsize = nrow(X_train).
Tuning was done with standard grid search over the 3 parameters of ntree, mtry, and nodesize. 
Descending order of variable importance is mileage, horsepower, model year, and displacement. 
